...
const LoginModal = ({ open, onClose, onSwitchToRegister }: LoginModalProps) => {
  ...
  useEffect(() => {
    if (pendingRedirect.current && user && role) {
      ...
      // Fetch profile for welcome toast
      supabase
        .from("profiles")
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error: invokeError } = await supabase.functions.invoke("custom-password-reset", {
        body: {
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            <div className="rounded-2xl border border-border bg-card pt-12 pb-8 px-8 shadow-[0_20px_60px_-15px_hsl(var(--primary)/0.15)]">
              {/* Close button */}
              <button
                onClick={onClose}
              ...
              {forgotMode ? (
                ...
                <>
                  ...
                  {error && (
                    <div className="mt-4 rounded-xl border border-destructive/30 bg-destructive/10 px-4 py-2.5 text-sm text-destructive flex items-start gap-2">
                  ...
                  {!forgotSent && (
                    <form onSubmit={handleForgotPassword} className="mt-6 space-y-5">
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          <input
                            ...
                            className="w-full rounded-xl border border-input bg-muted/50 px-4 py-3 pr-10 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary transition-colors"
                          />
                      ...
                      <button
                        ...
                        className="w-full rounded-xl gradient-bg py-3.5 text-sm font-bold text-primary-foreground shadow-lg hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2"
                      >
                ...
                <>
                  ...
                  {error && (
                    <motion.div
                      ...
                      className="mt-4 rounded-xl border border-destructive/30 bg-destructive/10 px-4 py-2.5 text-sm text-destructive flex items-start gap-2"
                    >
                      <AlertTriangle className="h-4 w-4 mt-0.5 shrink-0" />
                      <span>{error}</span>
                    </motion.div>
                  ...
                  <form onSubmit={handleSubmit} className="mt-6 space-y-5">
                    ...
                    <button
                      type="submit"
                      disabled={loading}
                      className="w-full rounded-xl gradient-bg py-3.5 text-sm font-bold text-primary-foreground shadow-lg shadow-primary/20 hover:opacity-90 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                    >
                      {loading ? (
                        <>
                  ...
                  {/* Privacy reassurance */}
                  <p className="mt-3 text-center text-[10px] text-muted-foreground/70 leading-relaxed">
