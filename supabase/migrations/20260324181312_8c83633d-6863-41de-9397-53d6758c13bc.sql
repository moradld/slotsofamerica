...
BEGIN
  ...
  -- Generate game prefix: first letter of each word in game name (uppercase), max 2 letters
  _game_prefix := upper(
    array_to_string(
      ARRAY(
        SELECT left(word, 1)
  ...
  IF length(_game_prefix) = 0 THEN
    _game_prefix := 'XX';
  END IF;

  _game_username := COALESCE(_username, 'unknown') || _game_prefix;
